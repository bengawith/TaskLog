    ...
    def add_task(self, task, date, priority):
        for existing_task in self.tasks:
            ...
            if similarity >= 0.8 and existing_task[1] == date:
                return False
        self.tasks.append([task, date, priority])
        self.save_tasks()
        return True
...
class TaskLoggerGUI:
    ...
    def load_tasks(self):
        ...
        for task, date, priority in self.task_logger.tasks:
            self.task_listbox.insert(tk.END, f"{task} (Due: {date}, Priority: {priority})")
    ...
    def add_task(self):
        ...
        priority = simpledialog.askstring("Priority", "Enter the priority for the task:")
        if task and date and priority:
            if self.task_logger.add_task(task, date, priority):
                self.load_tasks()
            else:
    ...
    def edit_task(self):
        ...
        if selected_task_index:
            ...
            task, date, priority = self.task_logger.tasks[index]
            ...
            new_priority = simpledialog.askstring("Edit Priority", "Enter the new priority:", initialvalue=priority)
            if new_task and new_date and new_priority:
                if self.task_logger.edit_task(index, new_task, new_date, new_priority):
                    self.load_tasks()
                else:
    ...
    def view_completed_tasks(self):
        completed_tasks = "\n".join([f"{task} (Due: {date}, Priority: {priority})" for task, date, priority in self.task_logger.complete])
    ...
    def search_tasks(self):
        ...
        if search_term:
            matching_tasks = [f"{task} (Due: {date}, Priority: {priority})" for task, date, priority in self.task_logger.tasks if search_term.lower() in task.lower()]
