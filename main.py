...
def update_task_list():
    ...
    for index, (task, date) in enumerate(tasks):
        task_label = CTkLabel(
            ...
            text_color="#FFFFFF" if index != selected_task_index else "#FFFFFF",
            font=("Arial", 12),
            corner_radius=8,
            anchor="w",
        )
...
def view_completed_tasks():
    ...
    completed_task_list = CTkScrollableFrame(master=completed_window, fg_color="#2B2B2B", width=350, height=250)
    completed_task_list.pack(fill="both", expand=True, padx=10, pady=10)

    update_completed_list()

    # Buttons for marking incomplete
    CTkButton(
        master=completed_window,
